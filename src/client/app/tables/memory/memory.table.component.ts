...
export class TableComponent implements OnInit {
  ...
  makeFilterQuery(param:any){
    ...
    this.showTypeAhead = false;
    this.http.post('http://localhost:3000/goldenregister/register', queryObj) // ...using post request
      .map((res) => res.json()) // ...and calling .json() on the response to return data
      .subscribe(message => {
        this.MemoryParamsService.clearTableRows();
        this.rows = [];
  ...
  onSearchChange(searchValue: string, column: string) {
    ...
    if (column === "Block Revision" && searchValue !== '') {
      this.blockRevisions=[];
      this.http.get('http://localhost:3000/goldenregister/register?block_revision' + searchValue) // ...using post request
        .map((res) => res.json()) // ...and calling .json() on the response to return data
        .subscribe(message => {
          this.showTypeAhead = true;
